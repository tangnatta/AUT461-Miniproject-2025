...
class Dataloader:
    ...
    def interpolate_columns(self, df: pd.DataFrame, cols_to_interpolate: List[str], group_by: str, time_index_column: str = "date") -> pd.DataFrame:
        """
        ...
        Args:
            ...
            time_index_column (str): The column to use as the time index for interpolation.
        ...
        # Ensure the time index column is in datetime format
        df_temp[time_index_column] = pd.to_datetime(
            df_temp[time_index_column], format='%Y-%m-%d', errors='coerce')

        # Process each group separately to avoid issues with duplicate indices
        for name, group in df_temp.groupby(group_by):
            # Remove duplicates in the time index within each group
            group = group.drop_duplicates(subset=[time_index_column])
            group = group.set_index(time_index_column).sort_index()
            ...
            for col in cols_to_interpolate:
                if col in group.columns:
                    # Create a mapping from date to interpolated value
                    value_map = group[col].to_dict()
